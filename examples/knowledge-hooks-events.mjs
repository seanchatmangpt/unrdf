...
async function basicEventHooks() {
  ...
  await runApp(async () => {
    ...
    const unregister1 = hooks.registerEventHook(
      ...
      async (payload) => {
        console.log(`✅ Quad added: ${payload.quad.subject.value} ${payload.quad.predicate.value} ${payload.quad.object.value}`);
        console.log(`   Context: ${payload.context.source} at ${payload.context.timestamp.toISOString()}`);
      },
      { id: 'quad-monitor' }
    );
    ...
    const unregister2 = hooks.registerEventHook(
      ...
      async (payload) => {
        console.log(`❌ Quad removed: ${payload.quad.subject.value} ${payload.quad.predicate.value} ${payload.quad.object.value}`);
      },
      { id: 'removal-monitor' }
    );
  ...
  await runApp(async () => {
    ...
    const unregister = hooks.registerEventHook(
      ...
      async (payload) => {
        ...
        console.log(`✅ ALLOWED: ${quad.subject.value} ${quad.predicate.value} ${quad.object.value}`);
