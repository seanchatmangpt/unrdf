...
async function example2_ManualFlow() {
  ...
  console.log(`   ✓ Stack: ${stackProfile.webFramework || 'unknown'} + ${stackProfile.testFramework || 'none'}`)

  // Step 3: Build project model
  console.log('\n3️⃣  Building project model...')
  const projectStore = buildProjectModelFromFs({ fsStore, baseIri: 'http://example.org/unrdf/project#' })
  ...
  console.log(`   ✓ Templates inferred: ${Object.values(templateResult.summary.byKind || {}).reduce((a, b) => a + b, 0)} total`)
...
async function example3_Materialization() {
  ...
  const { projectStore, domainStore } = result.state
...
async function example4_StateAccess() {
  ...
  const { fsStore, projectStore, domainStore, templateGraph, snapshot } = result.state
