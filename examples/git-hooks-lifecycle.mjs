...
async function gitHooksLifecycleExample() {
    ...
    registerHook(engine, {
      ...
      action: async (payload, ok) => {
        if (
          ...
          payload.data.predicate.value === 'http://git/commit/hash'
        ) {
          lifecycleEvents.push({
            phase: 'post-commit',
            allowed: ok,
    ...
    registerHook(engine, {
      ...
      action: async (payload, ok) => {
        if (
          ...
          payload.data &&
          payload.data.predicate &&
          payload.data.predicate.value === 'http://git/push/ref'
        ) {
          lifecycleEvents.push({
            phase: 'post-receive',
            allowed: ok,
          ...
          const actions = [
            ...
            },
            {
              type: 'notify',
              channel: '#deployments',
              message: `New push to ${payload.data.object.value}`,
            },
            {
              type: 'backup',
              repository: 'main',
  ...
  for (const file of stagedFiles) {
    const fileRes = await ingress.fromJSON(GitFile, JSON.stringify(file), {
      base: 'http://git/file/',
  ...
  const commitRes = await ingress.fromJSON(GitCommit, JSON.stringify(commitData), {
  ...
  const pushRes = await ingress.fromJSON(GitPush, JSON.stringify(pushData), {
    base: 'http://git/push/',
  ...
  lifecycleEvents.forEach((event, index) => {
    console.log(
      `   ${index + 1}. ${event.phase}: ${event.allowed ? '✅' : '❌'} (${event.timestamp})`
    );
  ...
  const commitExport = await egress.toJSON(GitCommit, engine.store, {
    subject: 'urn:git:commit:abc123def456',
  });
  ...
  const turtleExport = await egress.toTurtle(null, engine.store, engine);
