...
async function demonstratePolicyPacks() {
  try {
    ...
    const complianceManifest = createPolicyPackManifest('sox-compliance', [
      {
        meta: {
    ...
    const governanceManifest = createPolicyPackManifest('data-governance', [
      {
        meta: {
          name: 'privacy-hook',
    ...
    const compliancePack = new (await import('../src/knowledge-engine/policy-pack.mjs')).PolicyPack(complianceManifest);
    const governancePack = new (await import('../src/knowledge-engine/policy-pack.mjs')).PolicyPack(governanceManifest);
    ...
    console.log(`  SOX Compliance (dev): ${complianceCompat.compatible ? '✅ compatible' : '❌ incompatible'}`);
    ...
    console.log(`  Data Governance (prod): ${governanceCompat.compatible ? '✅ compatible' : '❌ incompatible'}`);
    ...
    console.log(`  SOX Compliance: ${complianceStats.hooks.total} hooks, loaded: ${complianceStats.hooks.loaded}`);
    
    const governanceStats = governancePack.getStats();
    console.log(`  Data Governance: ${governanceStats.hooks.total} hooks, loaded: ${governanceStats.loaded}`);
