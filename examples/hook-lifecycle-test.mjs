...
import { motionComplianceHook, financialMonitoringHook, dataQualityHook } from './define-hook-example.mjs';
...
async function testHookValidation() {
  ...
  try {
    const fullHook = defineHook({
      ...
      run: async (event) => ({ result: 'full-success' }),
      after: async (event) => ({ result: { finalStatus: 'completed' } })
    });
...
async function testEdgeCases() {
  ...
  try {
    const largePayload = {
      ...
      metadata: Array(1000).fill(0).map((_, i) => ({ id: i, value: `item-${i}` }))
    };
  ...
  try {
    ...
    const beforeResult = await motionComplianceHook.before(circularEvent);
...
async function testPerformance() {
  ...
  try {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      promises.push(
        financialMonitoringHook.before(event)
          .then(result => financialMonitoringHook.run({ ...event, payload: { ...event.payload, ...result } }))
          .then(result => financialMonitoringHook.after({ ...event, result, cancelled: false }))
      );
    }
