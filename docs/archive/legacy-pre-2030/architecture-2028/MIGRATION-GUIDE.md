# Migration Guide: UNRDF vlatest → 2028
...
This guide helps you migrate from UNRDF vlatest to UNRDF 2028 architecture. The good news: **zero breaking changes**. All 2028 features are opt-in enhancements.
...
```javascript
// vlatest code continues to work exactly as before
...
### Path 1: Stay on vlatest (No Migration)
...
**Action:** None required. Continue using vlatest APIs.
...
# Check which vlatest features you use
...
| vlatest Feature | 2028 Status | Breaking Changes |
|----------------|-------------|------------------|
...
```
unrdf
├── knowledge-engine       # ← Unchanged (vlatest compatible)
...
| Configuration | vlatest | 2028 (base) | 2028 (all features) |
|---------------|--------|-------------|---------------------|
...
| Operation | vlatest | 2028 (base) | 2028 (with features) |
|-----------|--------|-------------|---------------------|
| Local Query | <50ms | <50ms | <50ms |
| Add Triple | <1ms | <1ms | <1ms (unencrypted) |
| Add Triple (encrypted) | N/A | N/A | <latestms (+20%) |
...
**Solution:**
```javascript
// 2028 hooks are fully compatible with vlatest
...
UNRDF 2028 maintains 100% backward compatibility with vlatest while offering powerful opt-in enhancements. You control the migration pace, adopting only the features you need.
