/**
 * @file Core substrate test (without external dependencies)
 * @module agent-9/test-core
 */

// Import only core modules (not oxigraph-adapter)
import {
  createOperation,
  validateOperation,
  isOperation
...
function testTransactions() {
  ...
  console.log('  ✓ Is in transaction:', isInTransaction(substrate));
  ...
  console.log('  ✓ Not in transaction:', !isInTransaction(substrate));
...
function testSnapshot() {
  ...
  // Try to modify frozen snapshot (should fail)
  try {
    apply(snap, [
      createOperation('INSERT', {
        subject: 'ex:ShouldFail',
        predicate: 'ex:value',
        object: '"fail"'
      })
    ]);
    console.log('  ✗ ERROR: Should not be able to modify frozen snapshot');
  } catch (error) {
    console.log('  ✓ Correctly prevented modification of frozen snapshot');
  }
...
function runTests() {
  console.log('╔═══════════════════════════════════════════════════╗');
  console.log('║   Substrate Adapter Layer - Core Tests           ║');
  console.log('╚═══════════════════════════════════════════════════╝\n');
