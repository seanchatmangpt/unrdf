# UNRDF P0+P1 Orchestration Plan
...
```
PARALLEL (8-10 hrs)
├─ KGC-001: Receipt Wrapper [8h] → BLOCKING
└─ KGC-002: Schema Generator [10h] → BLOCKING

SEQUENTIAL (16 hrs)
└─ KGC-003: v6-compat Package [16h, deps: KGC-001+KGC-002] → BLOCKING

SEQUENTIAL (2 hrs)
└─ KGC-004: Workspace Update [2h, deps: KGC-003]
...
| ------- | ---------- | ---------- | -------------------------------------------------------------------------------- |
| KGC-001 | 🟡 PARTIAL | ~40%       | withReceipt uses Date.now() (non-deterministic), no KGC-4D integration, no tests |
| KGC-002 | 🟡 PARTIAL | ~60%       | Functions exist, no CLI tool, no tests, not applied to core packages             |
| KGC-003 | 🟡 PARTIAL | ~50%       | Package exists, no tests, no migration guide, ESLint rules untested              |
| KGC-004 | 🟢 DONE    | 100%       | None (v6-compat package exists)                                                  |

---

## Phase 1: P0 Critical Path (4 capsules, 36 hours)

### KGC-001: KGC-4D Receipt Wrapper HOF
...
**Effort**: 8 hours
**Dependencies**: None
**Parallel**: Yes (can run with KGC-002)
...
### KGC-003: @unrdf/v6-compat Package Completion

**Capsule ID**: KGC-003
**Priority**: P0
**Status**: 🟡 PARTIAL (50% complete)
**Effort**: 16 hours
**Dependencies**: V6-001 (receipt wrapper), KGC-002 (schema generator)
**Parallel**: NO (sequential after V6-001 + KGC-002)
...
```json
{
  ...
  "otel_score": "≥80/100",
  "dependencies": ["V6-001", "KGC-002"]
}
```

---

### KGC-004: Workspace Update

**Capsule ID**: KGC-004
**Priority**: P0
**Status**: 🟢 DONE (100% complete)
**Effort**: 2 hours (verification only)
**Dependencies**: KGC-003
**Parallel**: NO
...
├─ OXI-0: @unrdf/oxigraph L3→L4 [10h]
├─ OXI-2: @unrdf/core L2→L3 [14h]
└─ OXI-5: @unrdf/kgc-4d L4→L5 [7h]

WAVE 2 (Parallel after OXI-2):
├─ OXI-3: @unrdf/core L3→L4 [12h, deps: OXI-2]
├─ OXI-6: @unrdf/hooks L2→L5 [25h, deps: V6-001, OXI-2]
├─ OXI-7: @unrdf/streaming L2→L5 [27h, deps: V6-001, OXI-2]
├─ OXI-8: @unrdf/federation L2→L5 [28h, deps: V6-001, OXI-2]
└─ OXI-9: @unrdf/cli L2→L5 [22h, deps: V6-001, OXI-2]

WAVE 3 (Sequential - Integration Tests):
├─ OXI-1: @unrdf/oxigraph L4→L5 [7h, deps: ALL OTHER L5]
└─ OXI-4: @unrdf/core L4→L5 [10h, deps: ALL OTHER L5]
```

### Critical Path

```
V6-001 (8h) ──┬─→ OXI-2 (14h) ──→ Wave 2 (max 28h) ──→ Wave 3 (10h)
              └─→ OXI-0 (10h)
...
1. **Hours 0-10**: V6-001 + KGC-002 (2 agents)
2. **Hours 10-26**: KGC-003 (1 agent) + OXI-0 + OXI-2 + OXI-5 (3 agents)
...
### OXI-1: @unrdf/oxigraph L4 → L5

**Priority**: P1
**Dependencies**: OXI-0, ALL other L5 packages
**Effort**: 7 hours
...
### OXI-3: @unrdf/core L3 → L4

**Priority**: P1
**Dependencies**: OXI-2
**Effort**: 12 hours
**Parallel**: Yes (Wave 2, after OXI-2)
...
### OXI-4: @unrdf/core L4 → L5

**Priority**: P1
**Dependencies**: OXI-3, ALL other L5
**Effort**: 10 hours
**Parallel**: NO (Wave 3, sequential)
...
### OXI-6: @unrdf/hooks L2 → L5

**Priority**: P1
**Dependencies**: V6-001, OXI-2
**Effort**: 25 hours
**Parallel**: Yes (Wave 2)
...
| --------------------------- | ------------------------------------------------------------------ | ----------- | ---------------------------------------------------------------- |
| **backend-dev**             | V6-001, KGC-002, KGC-003, OXI-2, OXI-3, OXI-6, OXI-7, OXI-8, OXI-9 | ~110h       | Deep KGC-4D knowledge, pure function design, receipt integration |
| **tester**                  | All test creation                                                  | ~60h        | Adversarial testing, fixture creation, determinism validation    |
| **code-analyzer**           | KGC-002 validation, KGC-003 ESLint                                 | ~15h        | Static analysis, pattern detection, coverage validation          |
| **system-architect**        | KGC-004, dependency resolution                                     | ~10h        | Workspace configuration, dependency management                   |
...
| ---------- | ---------------------------- | ---------------------------------- | ---------------------------------------------------- |
| **Gate 1** | V6-001 + KGC-002             | Receipt wrapper + schema generator | Tests 100% pass, no Date.now(), schemas generated    |
| **Gate 2** | KGC-003                      | v6-compat complete                 | 20+ tests pass, OTEL ≥80/100, migration guide exists |
| **Gate 3** | KGC-004                      | Workspace functional               | pnpm install works, no regressions                   |
| **Gate 4** | Wave 1 (OXI-0, OXI-2, OXI-5) | L3 maturity baseline               | Deterministic tests pass, receipts integrated        |
| **Gate 5** | Wave 2 (OXI-3, OXI-6-019)    | L4 maturity                        | Adversarial tests pass, timeout guards enforced      |
| **Gate 6** | Wave 3 (OXI-1, OXI-4)        | L5 maturity                        | Integration tests pass, OTEL ≥80/100 for all         |
...
```json
{
  ...
  "artifacts": ["/packages/kgc-4d/src/wrapper.mjs", "/packages/kgc-4d/test/wrapper.test.mjs"],
  "agent": "backend-dev-1",
  "duration_hours": 8.2,
  "status": "COMPLETE"
}
...
**Storage**: `/home/user/unrdf/.receipts/capsule-<ID>.json`
**Chain Validation**:

```bash
timeout 5s node /home/user/unrdf/scripts/validate-receipt-chain.mjs V6-001 OXI-9
...
| KGC-002 | Schema Generator  | 🟡 IN PROGRESS | 60%      | 0/10  | -    | No CLI, no tests       |
| KGC-003 | v6-compat Package | 🔴 BLOCKED     | 50%      | 0/20  | -    | Awaits V6-001, KGC-002 |
| KGC-004 | Workspace Update  | 🟢 DONE        | 100%     | N/A   | N/A  | None                   |

## P1 Core Packages (0/10 capsules complete)

| ID    | Package    | Current | Target | Progress   | Tests | OTEL | Blocker              |
| ----- | ---------- | ------- | ------ | ---------- | ----- | ---- | -------------------- |
| OXI-0 | oxigraph   | L3      | L4     | 🔴 BLOCKED | 0/100 | -    | Awaits V6-001        |
| OXI-1 | oxigraph   | L4      | L5     | 🔴 BLOCKED | 0/10  | -    | Awaits ALL L4→L5     |
| OXI-2 | core       | L2      | L3     | 🔴 BLOCKED | 0/50  | -    | Awaits V6-001        |
| OXI-3 | core       | L3      | L4     | 🔴 BLOCKED | 0/30  | -    | Awaits OXI-2         |
| OXI-4 | core       | L4      | L5     | 🔴 BLOCKED | 0/15  | -    | Awaits ALL L4→L5     |
| OXI-5 | kgc-4d     | L4      | L5     | 🔴 BLOCKED | 0/15  | -    | Awaits V6-001        |
| OXI-6 | hooks      | L2      | L5     | 🔴 BLOCKED | 0/40  | -    | Awaits V6-001, OXI-2 |
| OXI-7 | streaming  | L2      | L5     | 🔴 BLOCKED | 0/45  | -    | Awaits V6-001, OXI-2 |
| OXI-8 | federation | L2      | L5     | 🔴 BLOCKED | 0/50  | -    | Awaits V6-001, OXI-2 |
| OXI-9 | cli        | L2      | L5     | 🔴 BLOCKED | 0/35  | -    | Awaits V6-001, OXI-2 |
...
| backend-dev-2 | KGC-002 | Create CLI tool         | 8h              | 3h                    |
...
| ------ | --------------- | ---------- | ---- | ------------------------- |
| Gate 1 | V6-001, KGC-002 | 🔴 BLOCKED | -    | Implementation incomplete |
| Gate 2 | KGC-003         | 🔴 BLOCKED | -    | Awaits Gate 1             |
| Gate 3 | KGC-004         | 🟢 READY   | N/A  | None                      |
...
KGC-004: COMPLETE (commit: 8a4ceae9, tests: N/A, otel: N/A)
...
DAY 1-2: [V6-001 ████░░] [KGC-002 ██████░░] ← IN PROGRESS
DAY 3-4: [KGC-003 ░░░░░░░░░░░░░░░░] ← BLOCKED
DAY 5: [KGC-004 ✓] ← DONE
...
🔴 **CRITICAL**: KGC-003 blocked on dependencies - critical path delayed
```
...
timeout 10s node /home/user/unrdf/scripts/generate-dashboard.mjs \
  --receipts /home/user/unrdf/.receipts/ \
...
| ----- | ----- | ------------------------ | ----------------------------------------------------- | ------------- |
| 1     | 0-8   | P0 parallel start        | V6-001 (8h), KGC-002 (8h of 10h)                      | V6-001 ✓      |
| 2     | 8-16  | P0 completion            | KGC-002 finish (2h), KGC-003 start (6h of 16h)        | KGC-002 ✓     |
| 3     | 16-24 | KGC-003 continues        | KGC-003 (8h)                                          | -             |
| 4     | 24-32 | KGC-003 finish + KGC-004 | KGC-003 finish (2h), KGC-004 (2h), Gate 3 validation  | P0 COMPLETE ✓ |
| 5-6   | 32-48 | Wave 1 parallel          | OXI-0 (10h), OXI-2 (14h), OXI-5 (7h)                  | Wave 1 ✓      |
| 7-10  | 48-80 | Wave 2 parallel          | OXI-3, OXI-6, OXI-7, OXI-8, OXI-9 (max 28h per agent) | Wave 2 ✓      |
| 11-12 | 80-96 | Wave 3 sequential        | OXI-1 (7h), OXI-4 (10h)                               | P1 COMPLETE ✓ |
...
3. Execute V6-001 and KGC-002 in parallel
4. Validate Gate 1 before proceeding to KGC-003
