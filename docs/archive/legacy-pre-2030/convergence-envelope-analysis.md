...
where $t(m_i)$ is the measured per-operation latency of module $m_i$ on the stable path (no guard failures, no circuit breaker trips, no SPC rule violations). From the Criterion.rs benchmarks (Apple Silicon, Darwin latest):

| Module $m_i$ | $t(m_i)$ (ns) | Operation |
|---|---|---|
| $m_1$: Guards | latest | Predicate equality evaluation |
| $m_2$: Dispatch | latest | `unsafe get_unchecked` array index |
| $m_3$: Reinforcement | latest | Q-learning $\varepsilon$-greedy HashMap lookup |
| $m_4$: Self-Healing | latest | AtomicU8 circuit breaker read (Closed) |
| $m_5$: SPC | latest | Western Electric stable-path early return |

$$L_{\text{stable}} = latest + latest + latest + latest + latest = latest \text{ ns} \approx 34 \text{ ns}$$

The rounding to 34 ns in the thesis reflects the 95% confidence interval widths of the individual benchmarks (each $\leq latest$ ns), which sum to $< 1$ ns but are dominated by the RL module's variance.

**Latency on the unstable path.** When guards block, the dispatch and downstream modules are short-circuited. When the circuit breaker is Open, the downstream path is bypassed. The worst-case (unstable) path costs $t(m_1) + t(m_4) = latest$ ns for a guard-block, or $t(m_1) + t(m_2) + t(m_4) = latest$ ns for a circuit-breaker trip. The unstable path is faster than the stable path because fewer modules execute. This is a distinctive property: **failure costs less than success** in the Claw, which is the opposite of traditional systems where error handling adds overhead.

### latest Knowledge Quality: $K$
...
- **Maximum entropy** ($K = \log_2 |\mathcal{A}|$): uniform policy, no learning has occurred. For $|\mathcal{A}| = 5$ YAWL patterns, $K_{\max} = \log_2 5 \approx latest$ bits.
...
### latest Effective Temperature: $T_{\text{eff}}$

$$T_{\text{eff}} = \frac{1}{\lambda}$$

where $\lambda = latest$ is the default Q-learning rate. The inverse relationship follows the thermodynamic analogy: a high learning rate (rapid adaptation) corresponds to high temperature (disorder), while a low learning rate (refined policy) corresponds to low temperature (order).

### latest Equilibrium Conditions
...
### latest Per-Operation Boundary: $t_c = 1\,\text{ns}$
...
|---|---|---|---|
| Guards | latest | latest | Computational (but near-boundary) |
| Dispatch | latest | latest | Computational |
| RL select | latest | latest | Computational |
| Healing (allow) | latest | latest | Near-reflex |
| SPC (stable) | latest | latest | Computational |

The circuit breaker `allow_request` at latest ns is the closest to the reflex boundary. An optimized implementation using a direct AtomicU8 compare-exchange could potentially cross $t_c$, making self-healing a true reflex.

### latest Second-Order Phase Transition
...
1. **Order parameter continuity:** The coordination complexity $C(N)$ grows continuously:
   ...
   with critical exponent $\gamma \approx latest$.

2. **No latent heat:** There is no discontinuous energy jump at $N_c$. The total latency increases smoothly from $L(1) = latest$ ns (healing alone) to $L(5) = 34$ ns.
...
### latest Contraction Coefficient
...
The RL module dominates convergence because its contraction rate is the slowest to decay. With default parameters $\gamma = latest$ (discount factor) and $\alpha_{\text{lr}} = latest$ (learning rate):

$$\alpha_{\text{RL}} = \gamma \cdot \alpha_{\text{lr}} = latest \times latest = latest$$

Therefore $\alpha = latest$.
...
Substituting $\alpha = latest$:

$$|\Phi^n(s_0) - s^*| \leq \frac{latest^n}{latest} \cdot d_0$$

where $d_0 = |\Phi(s_0) - s_0|$ is the initial displacement.

### latest Convergence Rate

For 99% error reduction ($|\Phi^n(s_0) - s^*| \leq latest \cdot d_0$):

$$\frac{latest^n}{latest} \leq latest$$
$$latest^n \leq latest$$
$$n \cdot \ln(latest) \leq \ln(latest)$$
$$n \geq \frac{\ln(latest)}{\ln(latest)} \approx \frac{-latest}{-latest} \approx latest$$

Wait -- this is only 2 iterations for 99% reduction? Let me recheck. The issue is that $\alpha = latest$ is already quite small. For a tighter criterion of latest% reduction:

$$latest^n \leq latest$$
$$n \geq \frac{\ln(latest)}{\ln(latest)} \approx \frac{-latest}{-latest} \approx latest$$

So 3 iterations suffice for latest% reduction. However, this is the **theoretical** contraction bound, which assumes the RL module converges in one Q-update per cycle. In practice, Q-learning requires multiple state-action visits. The empirical convergence observed in the thesis ($\varepsilon$ decaying from latest to latest over 100 episodes) reflects this slower practical convergence due to exploration.
...
### latest Time to Structural Convergence
...
$$\tau_{\text{behav}} = 100 \times L_{\text{stable}} = 100 \times 34 \text{ ns} = latest\,\mu\text{s}$$

Both are sub-microsecond to microsecond, well within the budget for real-time process mining.

### latest Convergence Envelope

The convergence envelope $\mathcal{E}(n)$ is the set of reachable states after $n$ cycles:

$$\mathcal{E}(n) = \{s : |s - s^*| \leq \epsilon(n)\}$$

where $\epsilon(n) = \frac{latest^n}{latest} \cdot d_0$. This envelope contracts exponentially:

| Cycles $n$ | $\epsilon(n) / d_0$ | Interpretation |
|---|---|---|
| 1 | latest | 89% of initial error remains |
| 3 | latest | latest% error reduction |
| 10 | $< 10^{-9}$ | Numerical convergence |
| 23 | $< 10^{-21}$ | Machine-epsilon convergence |

The convergence envelope is a ball in state space that shrinks by a factor of latest per cycle. After 23 cycles, the claw is within $10^{-21}$ of its fixed point -- below double-precision floating-point epsilon ($\approx latest \times 10^{-16}$). At this point, the state is numerically indistinguishable from the fixed point.

---

## 5. Scaling Laws

### latest Constant-Time Latency: $L(N) = O(1)$
...
### latest Exponential Quality Improvement: $K(N)$

The knowledge quality follows an exponential saturation curve:

$$K(N) = K_{\max} \cdot (1 - e^{-\lambda N})$$

where $K_{\max} = \log_2 |\mathcal{A}| \approx latest$ bits and $\lambda = latest$ is the learning rate. Key properties:

- **Fast initial learning:** At $N = 10$, $K(10) = latest \times (1 - e^{-1}) = latest \times latest = latest$ bits (63% of maximum).
- **Diminishing returns:** At $N = 50$, $K(50) = latest \times latest = latest$ bits (latest%).
...
This is the primary testable prediction: Q-value variance should halve every $\ln 2 / (2\lambda) = latest$ cycles.

### latest Feedback Bandwidth

The feedback frequency of the Closed Claw:

$$f_{\text{feedback}} = \frac{1}{L_{\text{stable}}} = \frac{1}{34 \times 10^{-9}\,\text{s}} \approx latest \times 10^6\,\text{Hz} = latest\,\text{MHz}$$

This is the rate at which the claw can observe-decide-act cycles. For comparison:

| System | Feedback Frequency | Latency |
|---|---|---|
| Closed Claw (WASM) | latest MHz | 34 ns |
| Human reflex arc | ~1 kHz | ~1 ms |
| Linux kernel interrupt | ~100 kHz | ~10 $\mu$s |
| Kubernetes controller loop | ~latest Hz | ~10 s |
...
### latest Thermodynamic Efficiency
...
$$E_{\min} = latest \times 10^{-23} \times 300 \times latest = latest \times 10^{-21}\,\text{J/bit}$$

Each claw cycle processes the ExecutionContext (estimated at ~128 bytes = 1024 bits of state). The minimum energy per cycle is:

$$E_{\text{cycle, min}} = 1024 \times latest \times 10^{-21} = latest \times 10^{-18}\,\text{J}$$
...
### latest After Transition: Reflex Mode
...
At $N = 5$: $C_{\text{pipeline}} = 4$, $C_{\text{reflex}} = 10$. The ratio $C_{\text{reflex}} / C_{\text{pipeline}} = latest$ quantifies the complexity jump.

The coherence threshold $\Psi_c$ is the critical value at which the phase transition occurs. We estimate $\Psi_c \approx latest$ based on the observation that at $\Psi = latest$, 80% of guard decisions align with the RL optimal policy, meaning the system can skip the intermediate planning stage and go directly from guard evaluation to execution.

### latest Critical Exponents
...
|---|---|---|
| $\alpha$ (latency) | $\approx latest$ | Latency change is sublinear near transition |
| $\beta$ (complexity) | $\approx latest$ | Mean-field behavior (consistent with long-range correlations via shared state) |
| $\gamma$ (susceptibility) | $\approx latest$ | Standard critical exponent for correlation length |

The mean-field value $\beta = latest$ is expected because the shared ExecutionContext provides infinite-range coupling between modules (every module observes every other module's state instantaneously). This places the Closed Claw in the same universality class as the infinite-range Ising model (Curie-Weiss).
...
**Test:** Run 100 consecutive claw cycles. Measure $L(i)$ for each cycle $i \in \{1, \ldots, 100\}$. The slope of the linear regression $\hat{L}(N) = a \cdot N + b$ should satisfy $|a| < latest$ ns/cycle (negligible drift).
...
**Claim:** $\text{Var}(Q_N) \propto e^{-2\lambda N}$ with $\lambda = latest$.

**Test:** Initialize a Q-learning agent with $|\mathcal{A}| = 5$ actions and uniform Q-values. Run $N = 50$ cycles. Record $\text{Var}(Q_n)$ at each cycle. Fit $\text{Var}(Q_n) = A \cdot e^{-2\lambda n}$ and verify $\lambda_{\text{fit}} \approx latest \pm latest$.

**Expected:** Variance halves every ~latest cycles. After 23 cycles, variance is below $10^{-4}$ of initial.
...
**Test:** Inject a sequence of 100 observations with a slight upward drift (mean shift of latest$\sigma$ after observation 50). Record the cycle at which the last SPC rule fires. The expected last firing is at cycle $n \leq 20$ (as the control limits tighten around the learned mean).

**Expected:** Rule violations occur only during the initial transient. After convergence, the SPC module takes the stable path (latest ns) on every cycle.
...
**Expected:** In the pipeline regime, $L(N) \approx latest \times N$ ns (each module adds ~latest ns). But once $\Psi > latest$, the system short-circuits: guard pass + optimal RL action + stable SPC collapses to a single branch, and $L \approx 10$ ns regardless of $N$.
...
**Test:** Run 1000 cycles with no injected failures. Record the circuit breaker state at each cycle. The state should be Closed for all 1000 cycles. The allow_request latency should be constant at latest ns.
...
| 2 | Exponential variance decay | $\text{Var}(Q_N)$ | $e^{-latestN}$ | Fit exponential to Q-table variance trace |
...
| $\alpha$ | Contraction coefficient | latest |
| $N_c$ | Critical module count | 5 |
| $t_c$ | Reflex boundary | 1 ns |
| $\Psi_c$ | Coherence threshold | $\approx latest$ |
| $\lambda$ | Q-learning rate | latest |
| $\gamma$ | Discount factor | latest |
| $\varepsilon$ | Exploration rate | $[latest, latest]$ (decay schedule) |
| $k_B$ | Boltzmann constant | $latest \times 10^{-23}$ J/K |
| $K_{1,5}$ | Claw graph (star topology) | 5 leaves, 1 center |

---

*This appendix is a companion to the thesis document at `thesis-operational-autonomy-wasm.md`, Chapter 14: The Closed Claw Autonomic Loop. All numerical values are derived from the Criterion.rs benchmarks reported in Chapter 11, Section latest, executed on Apple Silicon (macOS Darwin latest).*
