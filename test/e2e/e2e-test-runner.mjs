...
import { createKubernetesManager, createK8sResources } from './k8s-utils.mjs';
...
describe('KGC Sidecar E2E Test Suite', () => {
  ...
  describe('Kubernetes Resources', () => {
    it('should verify namespace exists', async () => {
      ...
      const namespace = await kubernetesManager.getResourceStatus(testNamespace, 'namespace', testNamespace);
    ...
    it('should verify deployment exists', async () => {
      ...
      const deployment = await kubernetesManager.getResourceStatus(testNamespace, 'deployment', 'kgc-sidecar');
      
      expect(deployment.metadata.name).toBe('kgc-sidecar');
      expect(deployment.spec.replicas).toBe(2);
      expect(deployment.spec.template.spec.containers[0].image).toContain('unrdf/kgc-sidecar:latest');
    ...
    it('should verify service exists', async () => {
      ...
      const service = await kubernetesManager.getResourceStatus(testNamespace, 'service', 'kgc-sidecar-service');
    ...
    it('should verify configmap exists', async () => {
      ...
      const configMap = await kubernetesManager.getResourceStatus(testNamespace, 'configmap', 'kgc-sidecar-config');
    ...
    it('should verify secret exists', async () => {
      ...
      const secret = await kubernetesManager.getResourceStatus(testNamespace, 'secret', 'kgc-sidecar-secrets');
    ...
    it('should verify service account exists', async () => {
      ...
      const serviceAccount = await kubernetesManager.getResourceStatus(testNamespace, 'serviceaccount', 'kgc-sidecar-sa');
    ...
    it('should verify role and role binding exist', async () => {
      ...
      const role = await kubernetesManager.getResourceStatus(testNamespace, 'role', 'kgc-sidecar-role');
      const roleBinding = await kubernetesManager.getResourceStatus(testNamespace, 'rolebinding', 'kgc-sidecar-binding');
    ...
    it('should verify ingress exists', async () => {
      ...
      const ingress = await kubernetesManager.getResourceStatus(testNamespace, 'ingress', 'kgc-sidecar-ingress');
    ...
    it('should verify HPA exists', async () => {
      ...
      const hpa = await kubernetesManager.getResourceStatus(testNamespace, 'hpa', 'kgc-sidecar-hpa');
    ...
    it('should verify network policy exists', async () => {
      ...
      const networkPolicy = await kubernetesManager.getResourceStatus(testNamespace, 'networkpolicy', 'kgc-sidecar-netpol');
    ...
    it('should verify PDB exists', async () => {
      ...
      const pdb = await kubernetesManager.getResourceStatus(testNamespace, 'pdb', 'kgc-sidecar-pdb');
  ...
  describe('Pod Status and Health', () => {
    it('should wait for deployment to be ready', async () => {
      ...
      const deployment = await kubernetesManager.waitForDeploymentReady(testNamespace, 'kgc-sidecar', testConfig.timeout);
  ...
  describe('E2E Functionality Tests', () => {
    it('should test health checks', async () => {
      ...
      const portForward = await kubernetesManager.portForward(testNamespace, 'kgc-sidecar-service', 3000, 3000);
    ...
    it('should test metrics endpoint', async () => {
      ...
      // Port forward to access the service
      const portForward = await kubernetesManager.portForward(testNamespace, 'kgc-sidecar-service', 8080, 8080);
