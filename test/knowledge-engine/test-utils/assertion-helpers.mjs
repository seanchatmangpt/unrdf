...
export function assertHookFailure(result, expectedError = null, message = 'Hook should fail') {
  ...
  if (expectedError && result.error && !result.error.includes(expectedError)) {
    throw new Error(`${message}: Expected error to contain "${expectedError}", got "${result.error}"`);
  }
}
 ...
 * @param {string} [message] - Custom error message
 */
export function assertHookResults(results, expectedSuccess, message = 'Hook results should match expectations') {
  ...
  if (results.length !== expectedSuccess.length) {
    throw new Error(`${message}: Expected ${expectedSuccess.length} results, got ${results.length}`);
  }
  
  results.forEach((result, index) => {
    const expected = expectedSuccess[index];
    if (result.success !== expected) {
      throw new Error(`${message}: Result ${index} expected success=${expected}, got ${result.success}`);
    }
  });
}
 ...
 * @param {string} [message] - Custom error message
 */
export function assertHookDuration(result, maxDurationMs, message = 'Hook should complete within time limit') {
  if (!result || typeof result !== 'object') {
    throw new Error(`${message}: Result is not an object`);
  }
 ...
 * @param {string} [message] - Custom error message
 */
export function assertHookMemory(result, maxMemoryMB, message = 'Hook should use reasonable memory') {
  if (!result || typeof result !== 'object') {
    throw new Error(`${message}: Result is not an object`);
  }
 ...
 * @param {string} [message] - Custom error message
 */
export function assertHookCancelled(result, expectedReason = null, message = 'Hook should be cancelled') {
  if (!result || typeof result !== 'object') {
    throw new Error(`${message}: Result is not an object`);
  }
  ...
  if (expectedReason && !result.cancelReason.includes(expectedReason)) {
    throw new Error(`${message}: Expected cancel reason to contain "${expectedReason}", got "${result.cancelReason}"`);
  }
}
 ...
 * @param {string} [message] - Custom error message
 */
export function assertHookPhase(result, expectedPhase, message = 'Hook should complete expected phase') {
  if (!result || typeof result !== 'object') {
    throw new Error(`${message}: Result is not an object`);
  }
 ...
 * @param {string} [message] - Custom error message
 */
export function assertHookData(result, expectedData, message = 'Hook result should contain expected data') {
  if (!result || typeof result !== 'object') {
    throw new Error(`${message}: Result is not an object`);
  }
  ...
  Object.entries(expectedData).forEach(([key, expectedValue]) => {
    ...
    if (result[key] !== expectedValue) {
      throw new Error(`${message}: Result.${key} expected "${expectedValue}", got "${result[key]}"`);
    }
  });
}
 ...
 * @param {string} [message] - Custom error message
 */
export function assertValidationError(error, expectedField, expectedMessage = null, message = 'Should throw validation error') {
  if (!error || !(error instanceof Error)) {
    throw new Error(`${message}: Expected Error instance, got ${typeof error}`);
  }
  
  if (!error.message.includes(expectedField)) {
    throw new Error(`${message}: Error message should contain field "${expectedField}", got "${error.message}"`);
  }
  
  if (expectedMessage && !error.message.includes(expectedMessage)) {
    throw new Error(`${message}: Error message should contain "${expectedMessage}", got "${error.message}"`);
  }
}
 ...
 * @param {string} [message] - Custom error message
 */
export function assertSecurityFailed(result, expectedViolation = null, message = 'Security check should fail') {
  if (!result || typeof result !== 'object') {
    throw new Error(`${message}: Result is not an object`);
  }
  ...
  if (expectedViolation && result.securityViolation && !result.securityViolation.includes(expectedViolation)) {
    throw new Error(`${message}: Expected violation "${expectedViolation}", got "${result.securityViolation}"`);
  }
}
