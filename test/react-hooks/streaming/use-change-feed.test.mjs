...
describe('useChangeFeed', () => {
  ...
  describe('Change Filtering', () => {
    ...
    it('should filter by subject', () => {
      const changes = [
        ...
        {
          id: '2',
          quads: [{ subject: { value: 'http://example.org/product2' } }],
        },
        {
          id: '3',
          quads: [{ subject: { value: 'http://example.org/product1' } }],
    ...
    it('should apply custom filter function', () => {
      ...
      const customFilter = change => {
        return (
          change.operation === 'insert' && change.quads.some(q => parseFloat(q.object.value) > 100)
        );
      };
  ...
  describe('Statistics Tracking', () => {
    it('should track total changes count', () => {
      ...
      updateStats([{ operation: 'insert' }, { operation: 'insert' }, { operation: 'delete' }]);
  ...
  describe('Recent Changes', () => {
    ...
    it('should handle request for more than available', () => {
      const changes = [{ id: 'change-1' }, { id: 'change-2' }, { id: 'change-3' }];
  ...
  describe('Replay Functionality', () => {
    ...
    it('should return empty array for invalid range', async () => {
      const allChanges = [{ id: '1', timestamp: '2024-01-01T00:00:00Z' }];
  ...
  describe('Operations Configuration', () => {
    it('should track only configured operations', () => {
      ...
      const tracked = allChanges.filter(c => config.operations.includes(c.operation));
    ...
    it('should default to insert and delete operations', () => {
      ...
      const tracked = allChanges.filter(c => defaultOperations.includes(c.operation));
