 ...
 * @description Integration script that generates real package documentation from discovered ontology data
 *
 * This is the working integration point between package RDF data and the documentation build pipeline.
 * It:
 ...
 *
 * Usage: node scripts/unrdf-generate-integration.mjs
 */
 ...
 *
 * Auto-generated from ontology (unrdf package sync)
...
function main() {
  ...
  for (const pkg of packages) {
    try {
      ...
      // Log sample
      if (generatedCount <= 3) {
        console.log(`  ✅ Generated ${readmePath.replace(projectRoot, '.')}`);
      }
    } catch (error) {
      console.warn(`  ⚠️  Failed to generate ${pkg.name}: ${error.message}`);
    }
  }

  if (packages.length > 3) {
    console.log(`  ✅ ... and ${packages.length - 3} more package docs\n`);
  }
...
`;
  ...
  console.log(`  4. Commit: git add . && git commit -m "chore(docs): sync package discovery and generated docs"`);
}
