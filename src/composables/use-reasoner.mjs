...
export function useReasoner(options = {}) {
  const { _timeoutMs = 30_000 } = options;
  ...
  return {
    ...
    infer(rules, options = {}) {
      ...
      const rulesStore = typeof rules === 'string' ? engine.parseTurtle(rules) : rules;
    ...
    inferSequence(ruleSets, options = {}) {
      ...
      for (let i = 0; i < ruleSets.length; i++) {
        const _stepStartSize = storeContext.store.size;
        const stepResult = this.infer(ruleSets[i], {
          addToStore,
          returnStats: false,
        });
