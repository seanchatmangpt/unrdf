...
export function useZod(options = {}) {
  ...
  return {
     ...
     */
    generateFromPattern(subjectPattern, options = {}) {
      const quads = store.getQuads ? store.getQuads(null, null, null, null) : Array.from(store);
      const patternQuads = quads.filter(q => 
        q.subject.value === subjectPattern || 
        q.subject.value.includes(subjectPattern)
      );
     ...
     */
    generateFromSHACL(shapeIRI, options = {}) {
      const quads = store.getQuads ? store.getQuads(null, null, null, null) : Array.from(store);
      const shapeQuads = quads.filter(q => 
        q.subject.value === shapeIRI ||
        q.subject.value.includes(shapeIRI)
      );
      ...
      for (const path of propertyPaths) {
        ...
        const propertyQuads = shapeQuads.filter(q => 
