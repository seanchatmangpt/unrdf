/**
 * @fileoverview usePrefixes composable - prefix management and CURIE operations with context
 * 
 ...
 * Now uses unctx for global prefix management.
 * 
 * @version 1.0.0
 * @author GitVan Team
 * @license MIT
...
export function usePrefixes(initialPrefixes = {}, options = {}) {
  ...
  // Use engine's prefix management - integrate with engine's prefix system
  ...
  if (initialPrefixes && typeof initialPrefixes === 'object') {
    for (const [prefix, uri] of Object.entries(initialPrefixes)) {
      ...
      // Also register with engine if it has prefix management
      if (engine.registerPrefix) {
        engine.registerPrefix(prefix, uri);
      }
    }
  }
  ...
  return {
    ...
    expand(curie) {
      ...
      if (colonIndex === -1) {
        // Return original string if not a CURIE
        return curie;
      }
