...
export function useNQuads(options = {}) {
  ...
  return {
     ...
     * 
     * @example
     * const isValid = nquads.validate(nquadsString);
     * if (isValid) {
     *   console.log("Valid N-Quads");
     * } else {
     *   console.log("Invalid N-Quads");
     * }
     */
    validate(nquads) {
      if (typeof nquads !== "string") {
    ...
    getStats(input) {
      ...
      } else if (input && typeof input.size === "number") {
        // Analyze Store
        const engineStats = engine.getStats(input);
        return {
          ...engineStats,
          totalQuads: input.size,
          totalLines: input.size, // Approximation
          emptyLines: 0
        };
      } else {
        throw new TypeError("[useNQuads] Input must be a string or Store");
      }
    },
    ...
    setBaseIRI(newBaseIRI) {
      ...
      baseIRI = newBaseIRI;
      // Note: Engine baseIRI is managed by the context system
      return this;
    },
