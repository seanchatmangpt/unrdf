...
export function useStreamProcessor(config = {}) {
  const { changes, start: startFeed, stop: stopFeed, isRunning: feedRunning } = useChangeFeed(config.changeFeed || {});
  ...
  const completeWindow = useCallback((keepOverlap = false) => {
    ...
    if (keepOverlap) {
      ...
      windowBufferRef.current = windowBufferRef.current.filter(e =>
  ...
  const getWindowsByTimeRange = useCallback((startTime, endTime) => {
