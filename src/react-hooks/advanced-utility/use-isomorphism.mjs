 ...
 * } = useIsomorphism();
 */
export function useIsomorphism(_config = {}) {
  const { engine } = useKnowledgeEngineContext();
  const [isIsomorphic, setIsIsomorphic] = useState(false);
  const [mapping, _setMapping] = useState(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);

  const checkIsomorphism = useCallback(
  ...
  const canonicalize = useCallback(
    async graphUri => {
      try {
        ...
        const canonical = quads.sort((a, b) => quadToString(a).localeCompare(quadToString(b)));
        setLoading(false);
        return canonical;
      } catch (err) {
        setError(err);
