  ...
  for (const node of graph) {
    ...
    if (node['@type']) {
      ...
      for (const type of types) {
        store.add(quad(subject, asNamedNode('http://www.w3.org/1999/02/22-rdf-syntax-ns#type'), asNamedNode(type)));
      }
    }
    ...
    for (const [predicate, objects] of Object.entries(node)) {
      ...
      for (const object of objectList) {
        const objectNode = typeof object === 'string' && (object.startsWith('http') || object.startsWith('urn:'))
          ? asNamedNode(object)
          : asLiteral(object);
 ...
 */
export const storeToRDFXML = (store, options = {}) => {
 ...
 * @returns {import('n3').Store} RDF store
 */
export const csvToStore = (csv, options = {}) => {
  const store = new Store();
 ...
 */
export const flattenStore = (store, options = {}) => {
 ...
 */
export const denormalizeStore = (store, options = {}) => {
  const subjects = new Map();
  ...
  for (const obj of data) {
    ...
    for (const [predicate, value] of Object.entries(obj)) {
      ...
      for (const val of values) {
        const objectNode = typeof val === 'string' && (val.startsWith('http') || val.startsWith('urn:'))
          ? asNamedNode(val)
          : asLiteral(val);
