 ...
 */
export const assessDataQuality = (store, options = {}) => {
...
const assessConsistency = (store, assessment) => {
  ...
  if (blankNodes.size > 0) {
    recommendations.push('Consider using named nodes instead of blank nodes for better interoperability');
  }
...
const assessAccessibility = (store, assessment) => {
  ...
  const undescribedSubjects = [...subjectsWithoutLabels].filter(s => !subjectsWithDescriptions.has(s));
  ...
  for (const quad of store) {
    if (quad.predicate.value === 'http://www.w3.org/2000/01/rdf-schema#label' && quad.object.language) {
...
export const fixQualityIssues = (store, options = {}) => {
  ...
  if (options.removeDuplicates) {
    const originalSize = fixedStore.size;
