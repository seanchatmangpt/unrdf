...
function docMatchesSource(docPath, sourcePath) {
  const docName = docPath.toLowerCase().replace(/\.mdx?$/, "").split("/").pop()
  const sourceName = sourcePath.toLowerCase().replace(/\.(tsx?|jsx?|mjs)$/, "").split("/").pop()
...
export const extractDocReferences = (options) => {
  ...
  return result.drifts.map(d => ({ docFile: d.docFile, sourceFile: d.sourceFile }))
}
