...
export function buildDependencyGraph(options) {
  ...
  for (const file of sourceFiles) {
    nodes.set(file, { file, imports: [], importedBy: [], depth: 0, isCircular: false })
  }
...
function isSourceFile(filePath) {
  return /\.(tsx?|jsx?|mjs)$/.test(filePath) && !filePath.includes(".test.") && !filePath.includes(".spec.")
}

function extractPathFromIri(iri) {
  const match = iri.match(/fs#(.+)$/)
...
function calculateDepths(nodes) {
  const entryPoints = Array.from(nodes.values()).filter(n => n.importedBy.length === 0).map(n => n.file)
...
function isEntryPoint(filePath) {
  return filePath.includes("index.") || filePath.includes("main.") || filePath.includes("app.") ||
