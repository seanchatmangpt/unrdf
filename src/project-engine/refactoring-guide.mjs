...
const RefactoringSuggestionSchema = z.object({
  type: z.enum(["extract-module", "split-file", "rename", "move", "consolidate", "remove-dead-code"]),
...
export function generateRefactoringGuide(options) {
  ...
  const files = fileQuads.map(q => {
    ...
    return { path: q.object.value, byteSize, lineCount: Math.round(byteSize / 40), role }
  ...
  for (const file of files) {
    if (file.lineCount > thresholds.maxFileSize) {
      suggestions.push({
        ...
        target: file.path,
        reason: "File has ~" + file.lineCount + " lines (threshold: " + thresholds.maxFileSize + ")",
...
function detectNamingIssue(filePath, role) {
  ...
  if (filename.length < 4 && !["app", "api", "db", "ui"].includes(filename.toLowerCase())) {
    return { reason: "Short filename may be unclear", suggestion: "Use descriptive names" }
  }
  if (role === "Component" && filename[0] && !filename[0].match(/[A-Z]/)) {
    return { reason: "Component files should use PascalCase", suggestion: "Rename to " + filename.charAt(0).toUpperCase() + filename.slice(1) }
  }
  return null
}
...
function detectPotentialDeadCode(files, projectStore) {
  ...
  const importedFiles = new Set(importedQuads.map(q => {
...
function isEntryPoint(filePath) {
  return filePath.includes("index.") || filePath.includes("main.") || filePath.endsWith("/page.tsx") || filePath.endsWith("/route.ts")
}
...
export const validateRefactoringPlan = (plan) => ({
  valid: true,
  errors: [],
})
