...
import { createPolicyPackManifest } from './knowledge-engine/policy-pack.mjs';
import { createKnowledgeHook } from './knowledge-engine/schemas.mjs';
  ...
  foaf:age 25 .`;
    ...
    hook: defineCommand({
      ...
      scaffold: defineCommand({
        meta: {
          name: 'scaffold',
          description: 'Generate scaffolding for hooks and policy packs'
        },
        subCommands: {
          hook: defineCommand({
            meta: {
              name: 'hook',
              description: 'Generate a new knowledge hook'
            },
            args: {
              name: {
                type: 'positional',
            ...
            async run(ctx) {
              const hookName = ctx.args.name;
              const hookType = ctx.args.type;
              
              console.log(`🔧 Generating knowledge hook: ${hookName}`);
              
              // Create hook directory
              const hookDir = join(process.cwd(), 'hooks', hookName);
              ...
              const hookDef = {
                meta: {
                  name: hookName,
                  description: `Generated knowledge hook: ${hookName}`,
                  version: '1.0.0',
                  author: 'unrdf-cli'
                },
                when: {
                  kind: hookType,
                  ref: {
                    uri: `file://${hookName}.rq`,
                    sha256: 'placeholder',
                    mediaType: hookType.startsWith('sparql') ? 'application/sparql-query' : 'text/turtle'
                  }
                },
                run: async (event) => {
                  console.log(`Hook ${hookName} executed`);
                  return { success: true };
                }
              };
              
              // Write hook file
              const hookFile = join(hookDir, `${hookName}.mjs`);
              const hookContent = `/**
 * Knowledge Hook: ${hookName}
 * Generated by UNRDF CLI
 */

export default ${JSON.stringify(hookDef, null, 2)};
`;
              
              await writeFile(hookFile, hookContent);
              
              // Generate condition file
              const conditionFile = join(hookDir, `${hookName}.rq`);
              let conditionContent = '';
              
              switch (hookType) {
                case 'sparql-ask':
                  conditionContent = `# SPARQL ASK query for ${hookName}
ASK WHERE {
  ?s ?p ?o .
}
`;
                  break;
                case 'sparql-select':
                  conditionContent = `# SPARQL SELECT query for ${hookName}
SELECT ?s ?p ?o WHERE {
  ?s ?p ?o .
}
LIMIT 10
`;
                  break;
                case 'shacl':
                  conditionContent = `# SHACL shapes for ${hookName}
@prefix sh: <http://www.w3.org/ns/shacl#> .
@prefix ex: <http://example.org/> .

ex:${hookName}Shape a sh:NodeShape ;
...
`;
              ...
              await writeFile(conditionFile, conditionContent);
              
              // Generate test file
              const testFile = join(hookDir, `${hookName}.test.mjs`);
              const testContent = `/**
 * Tests for knowledge hook: ${hookName}
 * Generated by UNRDF CLI
 */
...
describe('${hookName} Hook', () => {
  test('should execute successfully', async () => {
    const result = await scenario('${hookName} execution')
      .setup(async () => {
        return createTestContext()
          .withStore(TestHelpers.createStore())
          .build();
      })
      .step('execute hook', async (context) => {
...
`;
          ...
          policy: defineCommand({
            meta: {
              name: 'policy',
              description: 'Generate a new policy pack'
            },
            args: {
              name: {
                type: 'positional',
            ...
            async run(ctx) {
              const policyName = ctx.args.name;
              
              console.log(`🔧 Generating policy pack: ${policyName}`);
              
              // Create policy pack directory
              const policyDir = join(process.cwd(), 'policy-packs', policyName);
              await mkdir(policyDir, { recursive: true });
              
              // Generate manifest
              const manifest = createPolicyPackManifest(policyName, [], {
                description: `Generated policy pack: ${policyName}`,
                author: 'unrdf-cli',
                license: 'MIT'
              });
              ...
              const readmeContent = `# ${policyName} Policy Pack
...
This policy pack contains the following hooks:
...
await manager.loadPolicyPack('${policyName}');
\`\`\`
...
`;
              ...
              console.log(`✅ Generated policy pack files:`);
              console.log(`  📄 ${manifestFile}`);
              console.log(`  📄 ${readmeFile}`);
              console.log(`  📁 ${hooksDir}`);
              console.log(`  📁 ${conditionsDir}`);
