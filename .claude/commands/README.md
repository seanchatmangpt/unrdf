...
Waste elimination - Identify and eliminate the 7 types of waste (Muda) in code and process. Focus on value-adding activities.
...
Design for Lean Six Sigma - Strategic methodology overview integrating waste elimination (Lean) and defect prevention (Six Sigma) from inception.

**Use when**: Understanding DfLSS methodology, understanding why DfLSS differs from DFSS, providing context for all other commands
...
Git workflow with strict validation - Add, Commit, Push with pre-validation checkpoints and "WARNINGS ARE ERRORS" policy.

**Use when**: Committing code, ensuring code quality before commit, validating changes
...
**Design**: `/dflss`, `/dmedi-design-process`, `/poka-yoke-design`, `/robust-design`, `/concept-selection`
...
**Medium (15-30 minutes)**: `/kaizen-improvement`, `/eliminate-muda`, `/eliminate-mura`, `/eliminate-muri`, `/expert-testing-patterns`, `/acp`

**Comprehensive (30+ minutes)**: `/dmaic-problem-solving`, `/dmedi-design-process`, `/root-cause-analysis`, `/fmea`, `/robust-design`, `/release-preparation`
...
- **Release Workflow**: `/release-preparation` → `/acp` → Deploy

## Workflow Chains

### Problem Discovery & Resolution
```
/andon-signals (detect problem)
  → /gemba-walk (understand root)
  → /root-cause-analysis (find cause)
  → /dmaic-problem-solving or /kaizen-improvement (fix)
  → /verify-tests (validate)
```

### New Feature Development
```
/voice-of-customer-qfd (capture needs)
  → /dmedi-design-process (design)
  → /fmea (assess risks)
  → /poka-yoke-design (prevent errors)
  → /expert-testing-patterns (test)
  → /acp (commit)
...
```
/80-20-fill-gaps (identify gaps)
  → /eliminate-muda / /eliminate-mura / /eliminate-muri (improve)
  → /kaizen-improvement (standardize)
  → /verify-tests (validate improvements)
  → /acp (commit)
```
...
6. **Quality Control**: Use `/verify-tests` and `/expert-testing-patterns` for rigorous testing
