/**
 * @file RDF Core Benchmark Suite Utilities
 * @module benchmarks/rdf-core/suite
...
export function saveReport(report) {
  try {
    ...
    const filename = `rdf-core-benchmarks-${timestamp}.json`;
...
export function formatReportForConsole(report) {
  let output = '\n';
  output += '═══════════════════════════════════════════════════════════════\n';
  output += '              RDF CORE PERFORMANCE REPORT\n';
  output += '═══════════════════════════════════════════════════════════════\n\n';
  ...
  for (const [name, result] of Object.entries(report.benchmarks)) {
    ...
    if (result.latency) {
      ...
      output += `  Latency: P50=${lat.p50?.toFixed(3)}ms, P95=${lat.p95?.toFixed(3)}ms, P99=${lat.p99?.toFixed(3)}ms\n`;
    }
    ...
    if (result.memory) {
      ...
      output += `  Memory: ${(mem.mean / 1024 / 1024).toFixed(2)}MB\n`;
    }
  ...
  if (report.regressions.length > 0) {
    ...
    for (const reg of report.regressions) {
      output += `⚠ ${reg.name}: +${reg.change.toFixed(1)}% (${reg.current.toFixed(3)} vs ${reg.baseline.toFixed(3)})\n`;
    }
    output += '\n';
  }
  ...
  if (report.improvements.length > 0) {
    ...
    for (const imp of report.improvements) {
      output += `↑ ${imp.name}: ${imp.change.toFixed(1)}% (${imp.current.toFixed(3)} vs ${imp.baseline.toFixed(3)})\n`;
    }
    output += '\n';
  }
