...
defmodule codemanufactoryRevops.Quality.purchaseApprovalCycle Time HoursMetric do
  @moduledoc """
  Purchase Approval Cycle Time Hours Quality Metric
  ...
  def start_link(opts \\ []) do
    ctx = OpenTelemetry.start_span(:start_metric, fn ->
      Logger.info("Starting Purchase Approval Cycle Time Hours Quality Metric")
  ...
  def record_event(event_type, metadata \\ %{}) do
    ctx = OpenTelemetry.start_span(:record_event, fn ->
      OpenTelemetry.set_attributes(ctx, %{
        "metric.name" => "Purchase Approval Cycle Time Hours",
        "metric.type" => "cycle_time_hours",
  ...
  @impl true
  def init(_opts) do
    Logger.info("Starting Purchase Approval Cycle Time Hours Metric")

    state = %{
      metric_name: "Purchase Approval Cycle Time Hours",
      metric_type: "cycle_time_hours",
      events: [],
      opportunities: 0,
      defects: 0,
      samples: [],
    ...
    emit_lifecycle_event("metric_start", %{"metric_type" => "cycle_time_hours"})
  ...
  def terminate(_reason, state) do
    Logger.info("Terminating Purchase Approval Cycle Time Hours Metric after #{state.calibration_count} calibrations")
...
defp calculate_metric_value(state) do
    if state.opportunities == 0, do: 0.0, else: Float.round(state.opportunities / state.opportunities, 2)
  end

  defp calculate_statistics(state) do
    %{
      cycle_time_hours: calculate_metric_value(state),
...
defp mean(list) when length(list) == 0, do: 0.0
  ...
  defp emit_lifecycle_event(activity, attributes) do
    event = %{
      ...
      "ocel:objectIds" => ["purchase_approval_cycle_time_hours_metric"],
