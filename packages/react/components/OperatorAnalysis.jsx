  ...
  return (
    <div className="space-y-6">
      ...
      <div className="chart-container overflow-x-auto">
        <h3 className="font-semibold mb-4 text-lg">Operator Details</h3>
        <table>
          <thead>
            <tr>
              <th>Operator</th>
          ...
          <tbody>
            {operators.map((op) => (
              <tr key={op.id}>
                ...
                <td className="text-sm text-slate-400 max-w-xs">{op.purpose}</td>
                <td className="font-semibold">{op.reduction} nats</td>
                <td className="font-semibold text-cyan-400">{op.latency}</td>
      ...
      <div className="chart-container space-y-4">
        <h3 className="font-semibold text-lg">Operator Cardinality Theorem</h3>
        <div className="space-y-4 text-sm">
          ...
          <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-4 space-y-3">
            <div className="font-mono text-cyan-400 text-sm">
              <p className="mb-2">n_min ≥ (H(λ) - H(A)) / C_max</p>
              <p className="mb-2">n_min ≥ (50 - 1) / 6.1</p>
              <p>n_min ≥ 8.11 ≈ 8 operators (necessary)</p>
            </div>
            <p className="text-slate-400 border-t border-slate-700 pt-3">
              Given: H(λ) = 50 nats (user intent entropy), H(A) = 1 nat (observable outcome
              entropy), C_max = 6.1 nats/operator (maximum channel capacity per operator)
            </p>
          </div>
      ...
      <div className="chart-container">
        <h3 className="font-semibold mb-4 text-lg">Information Flow Through Operator Chain</h3>
        <div className="space-y-2">
          {operators.map((op, idx) => (
            <div key={op.id}>
              <div className="flex items-center gap-4 mb-1">
                <span className="font-mono font-bold text-cyan-400 w-8">{op.id}</span>
                <span className="font-semibold flex-1">{op.name}</span>
                <span className="text-sm text-slate-400 w-24 text-right">{op.entropy}</span>
              </div>
