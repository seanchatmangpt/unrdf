...
import { useState, useCallback, _useMemo, useEffect } from 'react';
...
export function useHTFFramework(config = {}) {
   ...
   */
  const generateThesisReport = useCallback(
    (_format = 'markdown') => {
      const sections = [];
      ...
      sections.push(
        `**Status:** ${validation.state.isConverged ? '✅ CONVERGED' : '⚠️ IN PROGRESS'}\n\n`
      );
      ...
      sections.push(
        `- **Critical Path:** ${scheduling.scheduling.criticalPath.path.length} shards\n`
      );
      ...
      sections.push(
        `- **Family Balance:** ${(profile.analysis.coherence.familyBalance * 100).toFixed(1)}%\n`
      );
      sections.push(
        `- **Family Coverage:** ${(profile.analysis.coherence.familyCoverage * 100).toFixed(1)}%\n`
      );
      sections.push(
        `- **Integration Strength:** ${(profile.analysis.coherence.integrationStrength * 100).toFixed(1)}%\n`
      );
      sections.push(
        `- **Completeness:** ${(profile.analysis.coherence.completeness * 100).toFixed(1)}%\n\n`
      );
    ...
    return {
      shards: {
        ...
        byFamily: Object.fromEntries(
          Object.entries(profile.analysis.profileByFamily).map(([family, prof]) => [
            family,
            prof.count,
          ])
        ),
      ...
      schedule: {
        totalWeeks:
          scheduling.scheduling.startDate && scheduling.scheduling.deadline
            ? Math.ceil(
                (scheduling.scheduling.deadline - scheduling.scheduling.startDate) /
                  (7 * 24 * 60 * 60 * 1000)
              )
            : 0,
