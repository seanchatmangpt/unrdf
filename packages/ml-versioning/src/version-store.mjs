...
export class MLVersionStore {
  ...
  async loadVersion(versionId) {
    const query = `
      ...
      WHERE {
        GRAPH ?g {
          <${ML_VERSION_NS}${versionId}> ml:modelJson ?modelJson ;
                          ml:metadata ?metadata ;
                          ml:metrics ?metrics ;
                          ml:hash ?hash ;
                          ml:timestamp ?timestamp .
          OPTIONAL { <${ML_VERSION_NS}${versionId}> ml:previousHash ?previousHash }
        }
      }
    `;

    const resultsArray = await this.store.query(query, { useDefaultGraphAsUnion: true });
    ...
    let modelJson = JSON.parse(result.modelJson.value);
    ...
    const model = await tf.models.modelFromJSON({ modelTopology: modelJson });
  ...
  async getVersionAtTime(modelId, timestamp) {
    ...
    const query = `
      ...
      WHERE {
        GRAPH ?g {
          ?version ml:modelId "${safeModelId}" ;
                   ml:timestamp ?timestamp ;
                   ml:versionId ?versionId .
          FILTER(xsd:integer(?timestamp) <= ${timestamp})
        }
      }
    ...
    const versionId = resultsArray[0].versionId.value;
  ...
  async getVersionHistory(modelId) {
    ...
    return resultsArray.map(r => ({
      ...
      hash: r.hash.value,
      previousHash: r.previousHash ? r.previousHash.value : null,
