...
export class SemanticQueryEngine {
  ...
  async initialize() {
    ...
    // Create vector index
    this.tempDir = path.join(os.tmpdir(), `vectra-index-${crypto.randomUUID()}`);
  ...
  async hybridSearch(nlQuery, sparqlPattern = null, options = {}) {
    ...
    // SPARQL filtering
    const sparqlQuery = `SELECT ?s ?p ?o WHERE { ?s ?p ?o . ${sparqlPattern} }`;
  ...
  /**
   * Cleanup resources, deleting the temporary vector index directory
   */
  async dispose() {
    if (this.tempDir) {
      try {
        ...
        console.error('Failed to cleanup SemanticQueryEngine index directory:', err);
      }
    }
  }
}
