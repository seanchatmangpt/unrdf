/**
 * @file YAWL Workflow RDF Integration - RDF serialization and deserialization
 * @module @unrdf/yawl/workflow/rdf
 *
 * @description
 * RDF integration for YAWL workflows using @unrdf/oxigraph:
 * - Serialize Workflow instances to RDF quads
 * - Deserialize RDF stores back to Workflow instances
 * - YAWL ontology mapping (split types, join types, task kinds)
 * - Named graph support for workflow organization
 *
 * @example
 ...
 * const workflow = new Workflow({ id: 'example', tasks: [...] });
 * const { specUri, quadCount } = workflowToRDF(workflow, store);
 *
 * const loaded = await workflowFromRDF(store, 'example');
 */
 ...
 * Supports named graphs for workflow organization.
 *
 ...
 * @returns {Object} Result with specUri, graph, and quad count
 *
 * @example
 * const { specUri, quadCount } = workflowToRDF(workflow, store);
 * console.log(`Serialized ${quadCount} quads for workflow ${specUri}`);
 */
 ...
 * Supports querying from named graphs or default graph.
 *
 * @param {Object} store - RDF store (from @unrdf/oxigraph)
