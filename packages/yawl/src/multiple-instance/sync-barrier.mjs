...
export const BarrierConfigSchema = z.object({
  ...
  /** Completion threshold (min number of successes) */
...
export class SyncBarrier {
  ...
  constructor(count, options = {}) {
    ...
    this.threshold = config.threshold ?? config.count;
  ...
  async arrive(completion) {
    ...
    // Check if threshold has been met (YAWL compliance)
    const successCount = this._arrivals.filter(a => !a.failed).length;
    if (successCount >= this.threshold || this._arrivals.length >= this.count) {
      this._complete();
    }
  }
  ...
  getState() {
    return {
      ...
      successes: this._arrivals.filter(a => !a.failed).length,
