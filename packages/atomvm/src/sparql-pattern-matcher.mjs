...
/**
 * SPARQL ASK query pattern
 * @constant {RegExp}
 */
const ASK_PATTERN = /ASK\s+WHERE\s*\{([^}]+)\}/is;
...
export class SPARQLPatternMatcher {
  ...
  async matchPattern(subject, predicate, object, filters = []) {
    return tracer.startActiveSpan('sparql.matchPattern', async span => {
      ...
      try {
        ...
        // Execute match
        const quadsIterator = this.store.match(sArg, pArg, oArg);
        const quads = Array.isArray(quadsIterator) ? quadsIterator : Array.from(quadsIterator);
        const results = [];
  ...
  async executeQuery(sparqlQuery) {
    return tracer.startActiveSpan('sparql.executeQuery', async span => {
      ...
      try {
        ...
        // Parse query
        const parsed = this._parseQuery(sparqlQuery);
        if (!parsed) {
          throw new SyntaxError('Failed to parse SPARQL query');
        }

        const { type, variables, patterns, filters } = parsed;
  ...
  _parseQuery(query) {
    ...
    let type = 'SELECT';
    let match = normalized.match(SELECT_PATTERN);

    if (!match) {
      match = normalized.match(ASK_PATTERN);
      if (match) {
        type = 'ASK';
      } else {
        return null;
    ...
    if (type === 'SELECT') {
      ...
      variables = varsStr === '*' ? ['*'] : varsStr.match(/\?\w+/g)?.map(v => v.slice(1)) || [];
    ...
    return { type, variables, patterns, filters };
  }
   ...
   * @param {string} sparqlQuery - SPARQL SELECT or ASK query
  ...
  _compileFilterToBeam(filterExpr, varMap) {
    ...
    // DATATYPE(?x) = <uri>
    const datatypeMatch = trimmed.match(/DATATYPE\(\?(\w+)\)\s*=\s*(.+)/i);
    if (datatypeMatch) {
      const varName = this._getBeamVarName(datatypeMatch[1], varMap);
      const dt = this._compilePatternTermToBeam(datatypeMatch[2].trim(), '');
      return `element(1, ${varName}) =:= literal, element(3, ${varName}) =:= ${dt}`;
    }

    // STRLEN(STR(?x)) >= value
    const strlenMatch = trimmed.match(/STRLEN\(STR\(\?(\w+)\)\)\s*(>=|<=|>|<|=)\s*(.+)/i);
    if (strlenMatch) {
      const varName = this._getBeamVarName(strlenMatch[1], varMap);
      const op = strlenMatch[2] === '=' ? '=:=' : strlenMatch[2];
      const val = strlenMatch[3].trim();
      return `byte_size(element(2, ${varName})) ${op} ${val}`;
    }

    // REGEX(STR(?x), "pattern")
    const regexMatch = trimmed.match(/REGEX\(STR\(\?(\w+)\),\s*"(.+)"\)/i);
    if (regexMatch) {
      const varName = this._getBeamVarName(regexMatch[1], varMap);
      const pattern = regexMatch[2];
      // In BEAM we'd use re:run, but in guards we can't. 
      // This might need to be a filter after the list comprehension if it's too complex.
      // For now, let's use a simple binary match if it's a simple pattern, or just placeholder.
      return `nomatch =/= re:run(element(2, ${varName}), <<"${pattern}">>)`;
    }
