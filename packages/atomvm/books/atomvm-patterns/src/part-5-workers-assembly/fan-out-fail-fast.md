...
FleetPulse has reached the layer where **fan-out with fail-fast** becomes necessary. The preceding patterns remain intact; this chapter adds one reversible capability to the working core.
...
`Parallel.all` launches real promise tasks with a shared `AbortSignal`. The first failure aborts cooperative siblings and returns `Result.err`.

## Executable AtomVM/JavaScript example

```javascript
const result = await Parallel.all(
  vehicleIds.map(id => signal => queryVehicle(id, signal)),
);
```
...
```erlang
{otp_pattern, fan_out_fail_fast, ok}
```
