...
FleetPulse has reached the layer where **state machines for complex protocols** becomes necessary. The preceding patterns remain intact; this chapter adds one reversible capability to the working core.
...
`StateMachine` separates mode, event, and data. `Transition.nextState`, `keepState`, and `stop` are the only admitted transition forms.

## Executable AtomVM/JavaScript example

```javascript
const vehicle = createVehicleStateMachine('V-1024');
vehicle.send(VehicleEvent.startTracking());
await vehicle.call(VehicleEvent.beginMaintenance());
```
