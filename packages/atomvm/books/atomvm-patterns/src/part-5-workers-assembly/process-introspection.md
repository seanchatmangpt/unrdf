...
FleetPulse has reached the layer where **process introspection** becomes necessary. The preceding patterns remain intact; this chapter adds one reversible capability to the working core.
...
`ProcSys` reads consistent state between transitions, suspends and resumes the real mailbox, exposes queue depth and counters, and returns transition receipts.

## Executable AtomVM/JavaScript example

```javascript
ProcSys.suspend(proc);
const state = await ProcSys.getState(proc);
const stats = ProcSys.statistics(proc);
ProcSys.resume(proc);
```
...
```erlang
{otp_pattern, process_introspection, ok}
```
