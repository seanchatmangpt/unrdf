...
FleetPulse has reached the layer where **timed messages** becomes necessary. The preceding patterns remain intact; this chapter adds one reversible capability to the working core.
...
`ProcTimer` uses real platform timers only to enqueue frozen messages. Business work still runs inside the target mailbox. Tests use real elapsed time—never fake timers.

## Executable AtomVM/JavaScript example

```javascript
const heartbeat = ProcTimer.sendInterval(
  10_000,
  tracker,
  immutableMessage('HEARTBEAT'),
);
```
