...
FleetPulse has reached the layer where **event broadcasting** becomes necessary. The preceding patterns remain intact; this chapter adds one reversible capability to the working core.
...
`EventManager` broadcasts through a real process. A crashing handler is terminated and evicted while healthy handlers continue receiving current and subsequent events.

## Executable AtomVM/JavaScript example

```javascript
const bus = EventManager.start('fleet-events');
bus.addHandler(alertHandler);
await bus.syncNotify(VehicleAlert);
```
...
```erlang
{otp_pattern, event_broadcasting, ok}
```
