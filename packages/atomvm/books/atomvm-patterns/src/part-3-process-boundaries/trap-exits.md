...
FleetPulse has reached the layer where **trap exits** becomes necessary. The preceding patterns remain intact; this chapter adds one reversible capability to the working core.
...
A process with `trapExits(true)` receives a frozen `ExitSignal` through its ordinary mailbox instead of dying with its linked peer.

## Executable AtomVM/JavaScript example

```javascript
coordinator.trapExits(true);
ProcLink.link(coordinator, worker);
// worker crash becomes ExitSignal in coordinator state.
```
