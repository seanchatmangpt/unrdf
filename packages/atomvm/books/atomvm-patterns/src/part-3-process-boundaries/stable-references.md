...
FleetPulse has reached the layer where **stable references** becomes necessary. The preceding patterns remain intact; this chapter adds one reversible capability to the working core.
...
`ProcRef` is an opaque stable handle. Supervisors atomically swap its delegate and increment its generation while callers retain the same reference.

## Executable AtomVM/JavaScript example

```javascript
const ref = stableVehicleRef('V-1014');
ref.swap(spawnVehicle('V-1014/restarted'));
...
```erlang
{otp_pattern, stable_references, ok}
```
