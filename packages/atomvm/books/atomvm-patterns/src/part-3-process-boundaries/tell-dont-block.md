...
FleetPulse has reached the layer where **tell, don't block** becomes necessary. The preceding patterns remain intact; this chapter adds one reversible capability to the working core.
...
`tell` performs a non-blocking enqueue. A single consumer processes messages in FIFO order and transition receipts preserve the observed sequence.

## Executable AtomVM/JavaScript example

```javascript
for (let n = 0; n < 100; n += 1) {
  counter.tell(CounterMsg.increment());
}
```
...
```erlang
{otp_pattern, tell_dont_block, ok}
```
