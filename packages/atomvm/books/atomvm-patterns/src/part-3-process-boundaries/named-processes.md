...
FleetPulse has reached the layer where **named processes** becomes necessary. The preceding patterns remain intact; this chapter adds one reversible capability to the working core.
...
`ProcRegistry` registers real `Proc` or `ProcRef` instances, rejects duplicate live names, and removes registrations when the active process terminates.

## Executable AtomVM/JavaScript example

```javascript
ProcRegistry.register('fleet-coordinator', coordinator);
const target = ProcRegistry.whereis('fleet-coordinator');
```
...
```erlang
{otp_pattern, named_processes, ok}
```
