...
FleetPulse has reached the layer where **sealed message protocols** becomes necessary. The preceding patterns remain intact; this chapter adds one reversible capability to the working core.
...
JavaScript does not have sealed interfaces, so the port uses closed tagged constructors and dispatch functions that throw on unknown variants. Adding a command without a case makes the executable test fail immediately.

## Executable AtomVM/JavaScript example

```javascript
const stopped = commandHandler(
  initialVehicleState(VehicleId('V-1002')),
  VehicleCommand.emergencyStop(VehicleId('V-1002')),
);
// Unknown tags are refused rather than silently ignored.
```
...
```erlang
{otp_pattern, sealed_message_protocols, ok}
```
