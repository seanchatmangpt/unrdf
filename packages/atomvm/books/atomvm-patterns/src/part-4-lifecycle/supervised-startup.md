...
FleetPulse has reached the layer where **supervised startup** becomes necessary. The preceding patterns remain intact; this chapter adds one reversible capability to the working core.
...
`ProcLib.startLink` requires explicit `initAck` before exposing the child. Initialization failure or missing acknowledgement returns `Result.err`.

## Executable AtomVM/JavaScript example

```javascript
const started = await ProcLib.startLink(
  initial,
  async (state, { initAck }) => { initAck(); return readyState; },
  loopHandler,
);
```
...
```erlang
{otp_pattern, supervised_startup, ok}
```
