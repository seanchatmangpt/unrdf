...
FleetPulse has reached the layer where **supervision trees** becomes necessary. The preceding patterns remain intact; this chapter adds one reversible capability to the working core.
...
`Supervisor` implements `one_for_one`, `one_for_all`, and true ordered `rest_for_one`. Restart topology is asserted through real process identity and generation changes.

## Executable AtomVM/JavaScript example

```javascript
const sup = fleetSupervisor(SupervisorStrategy.REST_FOR_ONE);
const a = sup.supervise('a', 0, counterHandler);
const b = sup.supervise('b', 0, counterHandler);
const c = sup.supervise('c', 0, counterHandler);
```
...
```erlang
{otp_pattern, supervision_trees, ok}
```
