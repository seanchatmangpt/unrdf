...
FleetPulse has reached the layer where **monitors for observation** becomes necessary. The preceding patterns remain intact; this chapter adds one reversible capability to the working core.
...
`ProcMonitor.monitor` installs a one-way lifecycle observer. The observer records normal or abnormal termination without sharing fate.

## Executable AtomVM/JavaScript example

```javascript
const ref = ProcMonitor.monitor(target, reason => {
  lifecycleEvents.push({ reason });
});
```
...
```erlang
{otp_pattern, monitors_observation, ok}
```
