...
describe('duplicate delivery and reordering', () => {
  it('collapses an identical duplicate edge delivery idempotently', () => {
    ...
    const twice = fromCodeGraphTables({
      ...t,
      edges: { ...t.edges, file_algorithm: [...t.edges.file_algorithm, { file_id: 1, concept_id: 10, confidence: 0.9 }] },
    });
  ...
  it('refuses a duplicate edge whose evidence conflicts (no order-dependent winner)', () => {
    const t = tables();
    const conflicting = [{ file_id: 1, concept_id: 10, confidence: 0.1 }, ...t.edges.file_algorithm];
    for (const rows of [conflicting, [...conflicting].reverse()]) {
      expect(() => fromCodeGraphTables({ ...t, edges: { ...t.edges, file_algorithm: rows } }))
  ...
  it('is invariant under file, concept and edge row permutation', () => {
    ...
    const permuted = {
      ...
      concepts: Object.fromEntries(Object.entries(t.concepts).map(([k, v]) => [k, [...v].reverse()])),
  ...
  it('orders parts by code point, not host locale', () => {
    const graph = fromCodeGraphTables({ files: [{ file_id: 'b' }, { file_id: 'B' }, { file_id: 'a' }, { file_id: '_' }] });
...
describe('malformed input is refused with a typed reason', () => {
  it('refuses non-scalar identities instead of stringifying them', () => {
    expect(() => fromCodeGraphTables({ files: [{ file_id: {} }] })).toThrow('REFUSED_NON_SCALAR_IDENTITY:file_id');
    expect(() => fromCodeGraphTables({ files: [{ file_id: [1] }] })).toThrow('REFUSED_NON_SCALAR_IDENTITY');
    expect(() => fromCodeGraphTables({ files: [{ file_id: Number.NaN }] })).toThrow('REFUSED_NON_SCALAR_IDENTITY');
    expect(() => fromCodeGraphTables({ files: [{ file_id: true }] })).toThrow('REFUSED_NON_SCALAR_IDENTITY');
  });

  it('refuses a numeric and string file id that collide canonically', () => {
    expect(() => fromCodeGraphTables({ files: [{ file_id: 1 }, { file_id: '1' }] })).toThrow('REFUSED_DUPLICATE_FILE:1');
  });

  it('refuses missing or non-object table containers', () => {
    expect(() => fromCodeGraphTables(null)).toThrow('tables must be an object');
    expect(() => fromCodeGraphTables({ files: [], concepts: null })).toThrow('concepts must be an object');
  ...
  it('refuses duplicate concepts and dangling concept edges', () => {
    const t = tables();
    expect(() => fromCodeGraphTables({ ...t, concepts: { ...t.concepts, domains: [{ concept_id: 20 }, { concept_id: '20' }] } }))
      .toThrow('REFUSED_DUPLICATE_CONCEPT:domain:20');
    expect(() => fromCodeGraphTables({ ...t, edges: { ...t.edges, file_domain: [{ file_id: 1, concept_id: 99 }] } }))
  ...
  it('refuses forged graphs whose semantic values are not objects', () => {
    ...
    expect(() => admitSemanticPartsGraph(graph)).toThrow('REFUSED_SEMANTIC_VALUE_NOT_OBJECT:codegraph:file:1:algorithm');
  ...
  it('refuses wrong schema, duplicate parts and duplicate semantic ids', () => {
    const graph = fromCodeGraphTables(tables());
    expect(() => admitSemanticPartsGraph({ ...graph, schema: 'unrdf.semantic-parts.v0' })).toThrow('REFUSED_SCHEMA');
    expect(() => admitSemanticPartsGraph({ ...graph, parts: [...graph.parts, graph.parts[0]] }))
    ...
    expect(() => admitSemanticPartsGraph(forged)).toThrow('REFUSED_DUPLICATE_SEMANTIC_ID:codegraph:file:1:wikidata:Q12105');
  });
});
...
describe('unauthorized action and stale subject', () => {
  ...
  it('refuses an unknown subject and an unknown candidate rather than answering false', () => {
    ...
    expect(() => substitutionSurvivesSemanticFalsifier(graph, '1', '404')).toThrow('REFUSED_UNKNOWN_CANDIDATE:404');
  });
  ...
  it('refuses a subject whose required axis is empty', () => {
    const graph = fromCodeGraphTables(tables());
    expect(() => findAlternatives(graph, '3', { requiredAxes: ['domain'] })).toThrow('REFUSED_SUBJECT_AXIS_EMPTY:domain');
  });
});
...
describe('query parameter boundaries', () => {
  ...
  it('refuses duplicate, unknown and empty required axes', () => {
    expect(() => findAlternatives(graph, '1', { requiredAxes: ['algorithm', 'algorithm'] })).toThrow('REFUSED_DUPLICATE_REQUIRED_AXIS');
    expect(() => findAlternatives(graph, '1', { requiredAxes: ['language'] })).toThrow('REFUSED_UNKNOWN_AXIS:language');
    expect(() => findAlternatives(graph, '1', { requiredAxes: [] })).toThrow('REFUSED_REQUIRED_AXES_EMPTY');
    expect(() => findAlternatives(graph, '1', { requiredAxes: 'algorithm' })).toThrow('requiredAxes must be an array');
  });

  it('refuses non-positive or non-integer minimumShared', () => {
    for (const bad of [0, -1, 1.5, Number.NaN, '1']) {
      expect(() => findAlternatives(graph, '1', { minimumShared: bad })).toThrow('REFUSED_MINIMUM_SHARED');
    }
  });

  it('excludes candidates below minimumShared', () => {
    expect(findAlternatives(graph, '1', { requiredAxes: ['algorithm', 'domain'], minimumShared: 2 })).toHaveLength(1);
    expect(findAlternatives(graph, '1', { requiredAxes: ['algorithm', 'domain'], minimumShared: 3 })).toHaveLength(0);
  });
});
