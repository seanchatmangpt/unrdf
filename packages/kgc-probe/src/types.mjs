...
export const SourceSchema = z
  .object({
    ...
    hash: z
      .string()
      .regex(SHA256_REGEX, 'Must be 64-char hex SHA-256')
      .describe('SHA-256 hash of source content'),
  })
  .refine(data => data.lineEnd >= data.lineStart, {
    message: 'lineEnd must be >= lineStart',
    path: ['lineEnd'],
  })
  .refine(data => !data.path.includes('..'), {
...
export const FrontmatterSchema = z
  .object({
    ...
    receipts: z
      .array(z.string().regex(SHA256_REGEX, 'Receipt ID must be 64-char hex'))
      .min(0)
      .max(1000)
      .describe('Array of receipt IDs [0-1000 items]'),
    bounds: BoundsSchema.describe('Resource limits'),
    views: z.array(DiatasisViewSchema).min(1).max(4).describe('Diataxis view types'),
    sources: z.array(SourceSchema).min(0).max(100).describe('Source file references'),
    version: z
      .string()
  ...
  .refine(data => new Date(data.lastProved) >= new Date(data.createdAt), {
...
export const ReceiptSchema = z.object({
  ...
  dependencies: z
    ...
    .optional()
    .describe('Receipt IDs this depends on'),
...
export const ArtifactSummarySchema = z
  .object({
    ...
    by_severity: z
      .record(z.enum(['critical', 'warning', 'info']), z.number().nonnegative())
      .describe('Count per severity'),
...
export const ArtifactSchema = z
  .object({
    ...
    shard_hash: z
      .string()
      .regex(/^[a-f0-9]{64}$/)
      .describe('Blake3 hash of all shards'),
    ...
    integrity: z
      .object({
        checksum: z
          .string()
          .regex(/^[a-f0-9]{64}$/)
          .describe('Blake3 hash of observations'),
