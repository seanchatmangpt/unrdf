...
describe('Semantic Query Engine', () => {
  ...
  describe('HNSW Index Module', () => {
    ...
    it('should search for similar vectors', async () => {
      ...
      const vector1 = new Float32Array(384).fill(0.11);
      const vector2 = new Float32Array(384).fill(0.9);
  ...
  describe('Semantic Query Executor', () => {
    ...
    it('should execute semantic CONSTRUCT query through Oxigraph', async () => {
      const oxigraph = new OxigraphStore(store.getQuads());
      const semanticStore = {
        getQuads: () => store.getQuads(),
        query: (sparql, options) => oxigraph.query(sparql, options),
      };
      await initializeSemanticIndex(semanticStore);
      ...
      const results = await executeSemanticQuery(semanticStore, sparql, {
        enableSemantic: true,
      ...
      expect(results.some(item => item.object?.value === 'Alice')).toBe(true);
    });
    ...
    it('should fall back to exact execution for ASK queries', async () => {
      ...
      `;
      const results = await executeSemanticQuery(store, sparql, { enableSemantic: true });
    ...
    it('should fall back to exact execution when semantic disabled', async () => {
      ...
      `;
      const results = await executeSemanticQuery(store, sparql, { enableSemantic: false });
      expect(results).toHaveProperty('type', 'select');
    });
  ...
  describe('Performance Benchmarks', () => {
    ...
    it('should benchmark embedding generation', async () => {
      const texts = [];
      for (let i = 0; i < 100; i++) texts.push(`Person ${i} works at ExampleOrg`);
    ...
    it('should benchmark HNSW index search', async () => {
      ...
      for (let i = 0; i < 1000; i++) {
        ...
        for (let j = 0; j < 384; j++) vector[j] = Math.random() * 2 - 1;
        vectors.set(`doc${i}`, vector);
      }
      const index = await buildHNSWIndex(vectors, { dimensions: 384, maxElements: 2000 });
  ...
  describe('Utility Functions', () => {
    ...
    it('should compute L2 distance', () => {
      ...
      expect(l2Distance(vec1, vec2)).toBeCloseTo(5, 5);
    });
