...
export const SafeStringSchema = z
  ...
  .min(1)
  .max(1000)
  .refine(val => !val.includes('\0'), { message: 'String contains null byte' })
  .refine(val => !/[<>]/.test(val), { message: 'String contains HTML characters' });
 ...
 */
export const SPARQLQuerySchema = z
  .string()
  .min(1)
  .max(10000)
  .refine(
...
export const SafeFilePathSchema = z
  ...
  .min(1)
  .max(255)
  .refine(val => !val.includes('..'), { message: 'Path contains directory traversal' })
  .refine(val => !val.includes('\0'), { message: 'Path contains null byte' })
  .refine(val => !/^[~/]/.test(val), { message: 'Absolute paths not allowed' });
 ...
 * Port number schema
 */
export const PortSchema = z.number().int().positive().min(1).max(65535);
...
export const SanitizedHTMLSchema = z
  .string()
  .refine(val => !/<script/i.test(val), { message: 'HTML contains script tags' })
  .refine(val => !/(onerror|onload|onclick)/i.test(val), {
