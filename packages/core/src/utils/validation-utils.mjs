...
export function validateStore(store) {
  ...
  for (const quad of store) {
    if (!validateTerm(quad.subject)) {
      issues.push({
        type: 'error',
        message: `Invalid subject: ${quad.subject}`,
        quad,
      });
    }
    if (!validateTerm(quad.predicate)) {
      issues.push({
        type: 'error',
        message: `Invalid predicate: ${quad.predicate}`,
        quad,
      });
    }
    if (!validateTerm(quad.object)) {
      issues.push({
        type: 'error',
        message: `Invalid object: ${quad.object}`,
        quad,
      });
    }
    if (quad.graph && !validateTerm(quad.graph)) {
      issues.push({
        type: 'error',
        message: `Invalid graph: ${quad.graph}`,
        quad,
      });
    }
  }
  ...
  for (const quad of store) {
    ...
    if (quadStrings.has(quadStr)) {
      issues.push({
        type: 'warning',
        message: `Duplicate quad: ${quadStr}`,
        quad,
      });
    }
...
export function validateCommonPatterns(store) {
  ...
  const labelQuads = store.getQuads(
    null,
    asNamedNode('http://www.w3.org/2000/01/rdf-schema#label'),
    null,
    null
  );
