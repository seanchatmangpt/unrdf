...
export const GossipMessageType = {
  ...
  JOIN: 'join',
  LEAVE: 'leave',
  BARK: 'bark', // Pack activation signal
};
 ...
 * @enum {string}
 */
export const BarkSignalType = {
  WATCH_BARK: 'watch_bark',   // Initial detection (e.g., circuit breaker)
  GUARD_BARK: 'guard_bark',   // Boundary pressure alert
  DETECT_BARK: 'detect_bark', // Confirmed threat pattern
  PROBE_BARK: 'probe_bark',   // Scent discovery/investigation
};
...
const GossipMessageSchema = z.object({
  ...
  barkType: z.nativeEnum(BarkSignalType).optional(),
...
export class MembershipManager extends EventEmitter {
  ...
  /**
   * Broadcast a 'Bark' signal to the pack
   * @param {Object} options - Bark options
   * @param {BarkSignalType} options.type - Type of bark
   * @param {string} options.component - Component that barked
   * @param {Object} [options.payload] - Additional data
   * @returns {Promise<void>}
   */
  async broadcastBark({ type, component, payload = {} }) {
    const message = {
      type: GossipMessageType.BARK,
      from: this.nodeId,
      barkType: type,
      component,
      payload,
  ...
  handleMessage(message) {
    ...
    // Process BARK signals
    if (message.type === GossipMessageType.BARK) {
      this.emit('barkReceived', message);
      // Trigger local pack coordination
      this.emit(`bark:${message.barkType}`, message);
    }
  }
